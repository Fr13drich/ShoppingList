...
def read_1_EBbook_recipe(pic):
    pic = image_preprocessing(pic)
    text = reader.readtext(image=pic, detail=1, paragraph=True)
    # ref = 'EB' + str(text[0][1]).split(sep=' ', maxsplit=1)[0]
    print(text)
...
def read_1_BCbook_recipe(pic):
    ...
    try:
        ingredients = reader.readtext(image="img_ingredients.jpg", detail=1, paragraph=True, y_ths=.5, height_ths=5)
    ...
    try:
        instructions = reader.readtext(image='img_recipe.jpg', detail=1, paragraph=True, height_ths=0.2, min_size=50) #, x_ths=0.002, min_size=200
...
# def CG2json(location='./CGrecipesPics/'):
...
def choose_strategy(location):
    # return 'read_1_' + location[0:2] + 'book_recipe'
    if location[2:4] == 'CG':
...
def pics2json(location):
    ...
    for _root, _dirs, files in os.walk(location):
        for name in files:
            print('Reading ' + str(name) + '...')
            ref, name, ingredients = strategy(location + name)
            parsed_ingredients = parse_ingredients(ingredients)
            outfile = 'json/' + ref + '.json'
            print(str(ref) + ', ' + str(name))
            print(parsed_ingredients)
